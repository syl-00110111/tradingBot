...
class DashboardUI:
    ...
    def make_dashboard(self, global_mode, config, bot_state, signal_arrival_times):
        now = datetime.now()
        now_ts = time.time()
    ...
    async def input_handler(self, core):
        """Non-blocking input check for the sequential loop."""
        key = self._get_key_non_blocking()
        if not key:
            return

        key = key.upper()
        sorted_symbols = sorted([s for s in core.bot_state.keys() if not s.startswith("_")])
        ...
        if key == 'Q':
            ...
            if sorted_symbols:
                ...
                core.log(f"Manual BUY triggered for {symbol}")
                await execute_buy(core.exchange, core.data_manager, core.engine, symbol, core.bot_state[symbol], core.config, core.available_assets, core.suspended_pairs)
        elif key == 'S':
            if sorted_symbols:
                symbol = sorted_symbols[self.selected_pair_index]
                core.log(f"Manual SELL triggered for {symbol}")
                positions = core.bot_state[symbol].get('positions', [])
                if positions:
                    await execute_sell(core.exchange, core.data_manager, core.engine, symbol, core.bot_state[symbol], core.config, 0)

class DashboardHandler(logging.Handler):
    def __init__(self, ui, duration=5):
        super().__init__()
        self.ui = ui
