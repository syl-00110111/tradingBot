...
# Global References for Resume Logic
global_executor = None
global_device = None
global_pattern_manager = None
            ...
            if order and 'id' in order:
                async with pending_orders_lock:
                    pending_orders[str(order['id'])] = {
                        ...
                        'side': 'buy',
                        'timestamp': time.time(),
                        'trigger_data': data.to_dict() if hasattr(data, 'to_dict') else data
                    }
...
async def resume_suspended_pairs(exchange, config, data_manager, engine):
    ...
    or were waiting for a free position slot.
    """
    # Give the exchange a moment to settle balance updates and order fills
    await asyncio.sleep(0.5)

    async with bot_lock:
        open_positions = data_manager.get_open_positions()

    # We re-analyze pairs that are suspended OR pairs that don't have an open position
    suspended_symbols = list(pair_suspensions.keys())
    all_symbols = list(config.get('pairs', {}).keys())
    candidate_symbols = [s for s in all_symbols if s not in open_positions]

    targets = list(set(suspended_symbols + candidate_symbols))
    ...
    for symbol in targets:
        async with analysis_lock:
            if symbol not in analysis_in_progress:
                analysis_in_progress.add(symbol)
                asyncio.create_task(analyze_and_trade_wrapper(
                    exchange, symbol, config, data_manager,
                    global_pattern_manager, engine, global_device, global_executor
                ))
...
async def main():
    ...
    # Store globals for resumption logic
    global global_executor, global_device, global_pattern_manager
    global_executor = executor
    global_device = device
    global_pattern_manager = pattern_manager
