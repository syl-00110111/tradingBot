...
STRATEGIES = [
    ...
    'moving_closes', 'ichimoku_cloud', 'parabolic_sar', 'rsi_support_resistance',
...
def get_common_indicators(df, device=torch.device('cpu')):
    ...
    if missing_basics:
        if use_acceleration:
            price_t = torch.tensor(df['close'].values, device=device, dtype=torch.float64)
...
def get_signals(df, mode_config, is_backtest=False):
    ...
    elif strategy == 'moving_closes':
        df = strategy_moving_closes(df, mode_config)
...
def handle_mc_strategies(df, strategy, config, is_backtest):
    ...
    if strategy == 'mc_mean_reversion':
        ...
        df['sma_20'] = sma20.fillna(df['close']) if sma20 is not None else df['close']
        df['returns'] = np.log(df['close'] / df['close'].shift(1).replace(0, 1)).fillna(0)
        df['volatility'] = df['returns'].rolling(window=20).std().fillna(0)
...
def strategy_moving_closes(df, config):
    ma9 = ta.ema(df['close'], length=9)
    df['ma_9'] = ma9.fillna(0) if ma9 is not None else 0
    ma21 = ta.ema(df['close'], length=21)
...
def strategy_ichimoku(df, config):
    ...
    if ichi_result is not None and len(ichi_result) > 0:
        ...
        df['tenkan'] = ichimoku.iloc[:, 0].fillna(df['close']) if ichimoku is not None else df['close']
        df['kijun'] = ichimoku.iloc[:, 1].fillna(df['close']) if ichimoku is not None else df['close']
        df['span_a'] = ichimoku.iloc[:, 2].fillna(df['close']) if ichimoku is not None else df['close']
        df['span_b'] = ichimoku.iloc[:, 3].fillna(df['close']) if ichimoku is not None else df['close']
    else:
...
def strategy_bollinger(df, config):
    ...
    if bb is not None and not bb.empty:
        df['bb_low'] = bb.iloc[:, 0].fillna(df['close']) if bb is not None and not bb.empty else df['close']
        df['bb_mid'] = bb.iloc[:, 1].fillna(df['close']) if bb is not None and not bb.empty else df['close']
        df['bb_high'] = bb.iloc[:, 2].fillna(df['close']) if bb is not None and not bb.empty else df['close']
    else:
...
def strategy_breakout_volume(df, config):
    ...
    df['ma_20'] = ma20.fillna(df['close']) if ma20 is not None else df['close']
...
def strategy_atr_breakout(df, config):
    a14 = ta.atr(df['high'], df['low'], df['close'], length=14)
...
def strategy_williams_r(df, config):
    w14 = ta.willr(df['high'], df['low'], df['close'], length=14)
...
def strategy_ema_rsi_volume(df, config):
    e9 = ta.ema(df['close'], length=9)
    df['ema_9'] = e9.fillna(df['close']) if e9 is not None else df['close']
    e21 = ta.ema(df['close'], length=21)
    df['ema_21'] = e21.fillna(df['close']) if e21 is not None else df['close']
    r14 = ta.rsi(df['close'], length=14)
    df['rsi'] = r14.fillna(50) if r14 is not None else 50
    v20 = ta.sma(df['volume'], length=20)
    df['vol_ma'] = v20.fillna(df['volume']) if v20 is not None else df['volume']
...
def strategy_market_regime(df, config):
    ...
    bb = ta.bbands(df['close'], length=20, std=2)
    df['bb_low'] = bb.iloc[:, 0] if bb is not None and not bb.empty else df['close']
    df['bb_high'] = bb.iloc[:, 2] if bb is not None and not bb.empty else df['close']
...
def strategy_liquidation_cascade(df, config):
    ...
    # Cascade: Price drops > 2% in one candle + Volume > 2x average close volume
...
def strategy_mvrv_proxy(df, config):
    ...
    Proxy: Price / 200-day Moving Close Average (Market Value to 'Realized' Value proxy).
    """
    rp200 = ta.sma(df['close'], length=200)
    df['realized_proxy'] = rp200.fillna(df['close']) if rp200 is not None else df['close']
    df['mvrv_proxy'] = df['close'] / df['realized_proxy'].replace(0, 1)
...
def strategy_adx_trend(df, config):
    ...
    """
    try: adx = ta.adx(df['high'], df['low'], df['close'])
    except: adx = None
...
def strategy_pairs_trading(df, config):
    ...
    Proxy: Asset vs moving close average of its own close price (Self-pairs trading/Mean reversion).
    """
    ma50 = ta.sma(df['close'], length=50)
    df['ma_50'] = ma50.fillna(df['close']) if ma50 is not None else df['close']
    df['z_score'] = (df['close'] - df['ma_50']) / df['close'].rolling(window=50).std().replace(0, 0.0001).fillna(0.0001)
...
def strategy_halving_cycle(df, config):
    ...
    df['ema_200'] = e200.fillna(df['close']) if e200 is not None else df['close']
    e50 = ta.ema(df['close'], length=50)
    df['ema_50'] = e50.fillna(df['close']) if e50 is not None else df['close']
...
def strategy_listing_surge(df, config):
    ...
    # Surge: Volume > 5x average close volume + Price breakout
...
def strategy_simple_ema(df, config):
    ...
    ef = ta.ema(df['close'], length=ema_fast)
    df['ema_f_strat'] = ef.fillna(df['close']) if ef is not None else df['close']
    es = ta.ema(df['close'], length=ema_slow)
    df['ema_s_strat'] = es.fillna(df['close']) if es is not None else df['close']
...
def strategy_simple_sma(df, config):
    ...
    sf = ta.sma(df['close'], length=sma_fast)
    df['sma_f_strat'] = sf.fillna(df['close']) if sf is not None else df['close']
    ss = ta.sma(df['close'], length=sma_slow)
    df['sma_s_strat'] = ss.fillna(df['close']) if ss is not None else df['close']
...
def strategy_double_ema(df, config):
    ...
    if (device.type != 'cpu') or torch.backends.mkldnn.enabled:
        ...
        ef = ta.ema(df['close'], length=ema_fast)
        df['ema_f'] = ef.fillna(df['close']) if ef is not None else df['close']
        es = ta.ema(df['close'], length=ema_slow)
        df['ema_s'] = es.fillna(df['close']) if es is not None else df['close']
...
def strategy_double_ema_macd_rsi(df, config):
    ...
    if (device.type != 'cpu') or torch.backends.mkldnn.enabled:
        ...
        rp = ta.rsi(df['close'], length=rsi_p)
